...
if __name__ == "__main__":
        ...
        for max_len in [16,32,64]:
            for sample_size in [64,128,256,512]:
                for lr in [1e-5, 1e-4, 1e-3]:
                    for reg_lambda in [0.8, 1., 1.2]:
                        for steps in [50,100,150]:
                            for step_size in [0.01, 0.1, 0.2]:
    # num_epochs = 60
    # max_len = 64
    # sample_size = 256
    # lr = 1e-5
    ...
    # step_size = 0.1
                                ...
                                print(f'num_epochs: {num_epochs} max_len: {max_len} sample_size: {sample_size} lr: {lr} reg_lambda: {reg_lambda} steps: {steps} step_size: {step_size}  ==>  Average reward {rewards.mean():0.4f} vs random {random_rewards.mean():0.4f}')

                                results.append(rewards.mean())
                                log.append(f'num_epochs: {num_epochs} max_len: {max_len} sample_size: {sample_size} lr: {lr} reg_lambda: {reg_lambda} steps: {steps} step_size: {step_size}  ==>  Average reward {rewards.mean():0.4f} vs random {random_rewards.mean():0.4f}')
...
print(f'best one: {log[results.argmax()]}')
